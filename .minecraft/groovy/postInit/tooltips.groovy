    ...
    if (hasChannels) {
        for (item in channels_list) {
            if (stack.isItemEqual(item)) {
                tooltip.add(net.minecraft.client.resources.I18n.format('gtf.ae2.channels.enabled'))
                break
            }
        }
    } else {
        for (item in channels_list) {
            if (stack.isItemEqual(item)) {
                tooltip.add(net.minecraft.client.resources.I18n.format('gtf.ae2.channels.disabled'))
                break
            }
        }
    }
